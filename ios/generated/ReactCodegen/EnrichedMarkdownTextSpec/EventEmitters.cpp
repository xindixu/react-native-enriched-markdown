...
void EnrichedMarkdownEventEmitter::onMentionPress(OnMentionPress event) const {
...
void EnrichedMarkdownTextEventEmitter::onMentionPress(OnMentionPress event) const {
  dispatchEvent("mentionPress", [event=std::move(event)](jsi::Runtime &runtime) {
    auto payload = jsi::Object(runtime);
    payload.setProperty(runtime, "url", event.url);
payload.setProperty(runtime, "text", event.text);
